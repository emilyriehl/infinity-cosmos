...
Given a simplicial set $X$, it is conventional to write $X_n$ for the set of $n$-\textbf{sim\-plices}, defined by evaluating at $[n] \in \Del$. This is implemented as a scoped notation ``\verbatim{_[n]}'' accessible with ``open Simplicial'' or ``open scoped Simplicial.''
...
\end{definition}
 ...
 To begin down this path, we must first verify the implicit assertion that has just been made. A proof of the following result, due to Johan Commelin, will appear in Mathlib soon (see \texttt{Wombat.lean} for now).
 ...
 \begin{definition}[homotopy relation on 1-simplices]\label{defn:1-simplex-htpy}
  ...
  \end{center}
   ...
   \begin{lemma}[homotopic 1-simplices in a quasi-category]\label{lem:qcat-1-simplex-htpy}
    \uses{defn:1-simplex-homotopy, defn:quasi-category}
...
\begin{center}
  \begin{tikzcd}[column sep=huge]
    \mathcal{Cat} \arrow[r, start anchor=353, end anchor=190, bend right, hook', "U"] \arrow[r, phantom, "\bot"] & \mathcal{rQuiv} \arrow[l, start anchor=170, end anchor=7, bend right, "F"' pos=.48]
  \end{tikzcd}
  \end{center}
\end{proposition}
...
\end{lem}
   ...
   \begin{center}
   \begin{tikzcd}[column sep=huge]
     \mathcal{Cat} \arrow[r, start anchor=353, end anchor=190, bend right, hook'] \arrow[r, phantom, "\bot"] & \mathcal{sSet} \arrow[l, start anchor=170, end anchor=7, bend right, "\ho"' pos=.48]
   \end{tikzcd}
   \end{center}
   \end{proposition}
   ...
   \begin{proposition}\label{prop:nerve-fully-faithful}
    \lean{nerveFunctor.fullyfaithful} \leanok
    \uses{defn:nerve}
