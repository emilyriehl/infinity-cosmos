...
* `CategoryTheory.Bicategory.Strict.CartesianClosed.const`: the constant-diagram 1-cell
  ...
  (`const_ihom`, `const_whiskerLeft_ihomHomFunctor`).
...
noncomputable def preservesColimitOfAdjunction {B : C} {u : A ⟶ B} {f : B ⟶ A}
  ...
  preserves {c} hc := by
    ...
    let p := (c.paste s).ofIso (eqToIso (const_ihom f)) (eqToIso (Strict.comp_id d).symm)
    have Hp : p.IsAbsKan :=
    ...
    have key : p ≅ t.paste (mapCocone d u c) := by
      ...
      simp only [p, t, s, adjJ, LeftLift.ofIso_unit, LeftLift.ofIso_lift, LeftLift.paste, mapCocone,
      ...
      rw [comp_whiskerLeft, const_whiskerLeft_ihomMapAdjunction_unit adj, whisker_assoc]
      simp [Strict.associator_eqToIso, Strict.rightUnitor_eqToIso]
    -- Cancel the absolute lifting `t` ([RV] Lemma 2.4.1, cancelation).
    exact fun _ h ↦ Ht.ofPaste (Hp.ofIsoAbsKan key) h
