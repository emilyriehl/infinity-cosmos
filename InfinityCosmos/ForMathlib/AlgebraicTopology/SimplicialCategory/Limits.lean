...
instance SimplicialCategory.HasConicalLimit_hasLimit (F : J ⥤ C) [HasConicalLimit SSet F] : HasLimit F := HasLimit.mk
  { cone := (getConicalLimitCone SSet F).cone,
    isLimit := ConicalLimitCone_isLimitCone SSet _ (getConicalLimitCone SSet F).isConicalLimit }
...
instance (priority := 100) SimplicialCategory.hasConicalLimitOfHasConicalLimitsOfShape {J : Type u₁} [Category.{v₁} J]
    [SimplicialCategory C] [HasConicalLimitsOfShape J SSet C] (F : J ⥤ C) : HasConicalLimit SSet F :=
  HasConicalLimitsOfShape.has_conical_limit F

instance SimplicialCategory.HasConicalLimitsOfShape_hasLimitsOfShape [HasConicalLimitsOfShape J SSet C] :
...
instance SimplicialCategory.hasConicalLimitEquivalenceComp {F : J ⥤ C} (e : K ≌ J) [HasConicalLimit SSet F] :
    HasConicalLimit SSet (e.functor ⋙ F) :=
  HasConicalLimit.mk SSet
    { cone := Cone.whisker e.functor (conicalLimit.cone SSet F)
      isConicalLimit := {
        isLimit := IsLimit.whiskerEquivalence (conicalLimit.isConicalLimit SSet F).isLimit e
        isConicalLimit := fun X ↦
          IsLimit.whiskerEquivalence ((conicalLimit.isConicalLimit SSet F).isConicalLimit X) e
        }
    }
...
-- see Note [lower instance priority]
instance (priority := 100) SimplicialCategory.hasConicalLimitsOfShapeOfHasLimits {J : Type u₁} [Category.{v₁} J]
    [SimplicialCategory C] [HasConicalLimitsOfSize.{v₁, u₁} SSet C] : HasConicalLimitsOfShape J SSet C :=
  HasConicalLimitsOfSize.has_conical_limits_of_shape J

instance SimplicialCategory.HasConicalLimitsOfSize_hasLimitsOfSize [HasConicalLimitsOfSize.{v₂, u₂, v, v, u} SSet C] :
...
instance (priority := 100) SimplicialCategory.hasSmallestConicalLimitsOfHasConicalLimits [HasConicalLimits SSet C] :
    HasConicalLimitsOfSize.{0, 0} SSet C := hasConicalLimitsOfSizeShrink.{0, 0} SSet C

instance SimplicialCategory.HasConicalLimits_hasLimits [HasConicalLimits SSet C] : HasLimits C :=
  HasConicalLimitsOfSize_hasLimitsOfSize C

instance SimplicialCategory.HasConicalLimits.has_conical_limits_of_shape {C : Type u} [Category.{v} C]
    [SimplicialCategory C] [HasConicalLimits SSet C] (J : Type v)
    [Category.{v} J] : HasConicalLimitsOfShape J SSet C :=
  HasConicalLimitsOfSize.has_conical_limits_of_shape J
...
instance SimplicialCategory.HasConicalTerminal_hasTerminal [hyp : HasConicalTerminal SSet C] : HasTerminal C := by
  infer_instance
...
instance SimplicialCategory.HasConicalProduct_hasProduct {I : Type w} (f : I → C) [HasConicalProduct SSet f] :
...
instance SimplicialCategory.HasConicalProducts_hasProducts [hyp : HasConicalProducts.{w, v, v, u} SSet C] :
    HasProducts.{w, v, u} C := by
  intro I
  constructor
  intro f
  have := hyp.has_conical_limits_of_shape I
  have : HasConicalLimit SSet f := by infer_instance
  exact HasConicalLimit_hasLimit f

instance SimplicialCategory.HasConicalProducts_hasConicalTerminal [hyp : HasConicalProducts.{0, v, v, u} SSet C] :
    HasConicalTerminal SSet C := hyp.has_conical_limits_of_shape PEmpty.{1}

instance SimplicialCategory.HasConicalProducts_hasConicalTerminal' [hyp : HasConicalProducts.{w, v, v, u} SSet C] :
    HasConicalTerminal SSet C := by
  have inst := hyp.has_conical_limits_of_shape PEmpty
  exact hasConicalLimitsOfShape_of_equivalence (J := Discrete PEmpty.{w+1}) SSet emptyEquivalence
...
instance SimplicialCategory.HasConicalPullback_hasPullback {X Y Z : C} (f : X ⟶ Z) (g : Y ⟶ Z)
    [HasConicalPullback SSet f g] : HasPullback f g := HasConicalLimit_hasLimit (cospan f g)

-- variable (C) in
-- abbrev HasConicalPullbacks : Prop := HasConicalLimitsOfShape WalkingCospan C

instance SimplicialCategory.HasConicalPullbacks_hasPullbacks [HasConicalPullbacks SSet C] : HasPullbacks C := by
  infer_instance
